...
int main()
{
   ...
   sleep(1);
    ...
    int len = 24;
    cout << "snding buffer:" << (char *)buffer << endl;
    for (int n = 0; n < len; n++) {
      unsigned char *c = &buffer[n];
      wiringPiSPIDataRW(CHANNEL, c, 1);
    }
   //   result = wiringPiSPIDataRW(CHANNEL, buffer, 24);

   //  cout << "result:"<< result << endl;
    cout << "from arduino buffer:" << (char *)buffer << endl;
    // int r = read(fd, read_buffer, 24);
    // cout << "fd.read:" << r << endl;
