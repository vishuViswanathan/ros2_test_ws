...
static const long SPI_SPEED = 57600;
static const long MSG_TIME_GAP_us = 100000;
void *connection_handler(void *);

char send_receive(unsigned char *data, int len) {
    // cout << "CHANNEL:" << CHANNEL << endl;
    // cout << "send_receive, The data:" << (char *)data << endl;
    for (int n = 0; n < len; n++) {
...
char send_receiveTEST(unsigned char *data, int len) {
    // cout << "CHANNEL:" << CHANNEL << endl;
    // cout << "send_receive, The data:" << (char *)data << endl;
...
void *connection_handler(void *socket_desc)
{
	...
	while( (read_size = recv(sock, client_message , 2000 , 0)) > 0 )
	{
        ...
        send_receive(buffer, 24);
        cout << "after send_receive :" << buffer << endl;
        write(sock , buffer , 24);
		usleep(MSG_TIME_GAP_us);
